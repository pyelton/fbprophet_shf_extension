...
class TestDiagnostics(TestCase):
        ...
        for p in [1, 10]:
            for h in [1, 3]:
                ...
                df_shf = diagnostics.simulated_historical_forecasts(
                    m, m.history, horizon=horizon, k=k, period=period)
    ...
    def test_simulated_historical_forecasts_logistic(self):
        ...
        m.fit(df)
        df_shf = diagnostics.simulated_historical_forecasts(
            m, m.history, horizon='3 days', k=2, period='3 days')
        # All cutoff dates should be less than ds dates
        self.assertTrue((df_shf['cutoff'] < df_shf['ds']).all())
        # The unique size of output cutoff should be equal to 'k'
    ...
    def test_simulated_historical_forecasts_default_value_check(self):
        ...
        df_shf1 = diagnostics.simulated_historical_forecasts(
            m, input_df = m.history, horizon='10 days', k=1)
        df_shf2 = diagnostics.simulated_historical_forecasts(
            m, m.history, horizon='10 days', k=1, period='5 days')
        self.assertAlmostEqual(
    ...
    def test_simulated_historical_forecasts_random_forest(self):
        self.__df['x'] = self.__df['y'] - 1
        self.__df.ds = self.__df.ds.apply(lambda x: pd.to_datetime(x))                            
        ...
        for p in [1, 10]:
            for h in [1, 3]:
                ...
                horizon = '{} days'.format(h)
                df_shf = diagnostics.simulated_historical_forecasts(
                    m, self.__df, horizon=horizon, k=k, period=period)
                # All cutoff dates should be less than ds dates
                self.assertTrue((df_shf['cutoff'] < df_shf['ds']).all())
                # The unique size of output cutoff should be equal to 'k'
                self.assertEqual(len(np.unique(df_shf['cutoff'])), k)
...
if __name__ == '__main__':
    testCase = TestDiagnostics()
    #testCase.test_simulated_historical_forecasts()
    #testCase.test_simulated_historical_forecasts_logistic()
    #testCase.test_simulated_historical_forecasts_extra_regressors()
    #testCase.test_simulated_historical_forecasts_default_value_check()
    #testCase.test_cross_validation()
    #testCase.test_cross_validation_default_value_check()
    testCase.test_simulated_historical_forecasts_random_forest()
